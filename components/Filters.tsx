  ...
  return (
    <aside className="sticky-top hidden lg:block pr-4 border-r">
      <div className="flex items-center justify-between py-2">
        ...
        <button className="text-blue-600 text-sm" onClick={()=>router.push(path)}>Limpar tudo</button>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2 py-3">
          {categories.map(c => (
            <label key={c} className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={current.cat.includes(c)} onChange={()=>toggle('cat', c)} />
              {c}
            </label>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 py-3">
          {sizes.map(s => (
            <label key={s} className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={current.size.includes(s)} onChange={()=>toggle('size', s)} />
              {s}
            </label>
          ))}
        </div>
      ...
      <details open className="py-3 border-t">
        ...
        <div className="grid grid-cols-2 gap-2 py-3">
          {colors.map(c => (
            <label key={c} className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={current.color.includes(c)} onChange={()=>toggle('color', c)} />
              {c}
            </label>
          ))}
        </div>
      ...
      <details open className="py-3 border-t">
        ...
        <div className="flex items-center gap-2 py-3 flex-wrap">
          <label className="text-sm">De R$ <input className="border rounded px-2 py-1 w-24" placeholder="0" defaultValue={current.min||''} onBlur={(e)=>setRange(e.currentTarget.value || undefined, current.max||undefined)} /></label>
          <label className="text-sm">Até R$ <input className="border rounded px-2 py-1 w-24" placeholder="999" defaultValue={current.max||''} onBlur={(e)=>setRange(current.min||undefined, e.currentTarget.value || undefined)} /></label>
        </div>
      </details>

      <div className="py-3 border-t flex items-center justify-between">
        <label className="text-sm">Ordenar por:</label>
        <select className="border rounded px-2 py-1" defaultValue={current.sort} onChange={e=>setSort(e.target.value)}>
