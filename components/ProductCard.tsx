...
export default function ProductCard({ p }: { p: Product }) {
  const perInstallment = (p.price / p.installments)
    .toFixed(2)
    .replace(".", ",");
  ...
  return (
    <article className="border rounded-lg overflow-hidden bg-white flex flex-col card-shadow">
      <Link
        href={`/produto/${p.id}`}
        className="block aspect-[3/4] overflow-hidden bg-slate-50 relative"
      >
        {p.discount ? (
      ...
      <div className="p-3 grid gap-1">
        ...
        <Link href={`/produto/${p.id}`} className="font-semibold leading-tight">
          {p.title}
        </Link>
        ...
        <div className="text-xs text-gray-600">
          ou {p.installments}x de R$ {perInstallment} sem juros
        </div>
      </div>
      <div className="px-3 pb-3 flex gap-2 flex-wrap">
        {p.sizes.map((s) => (
          <span key={s} className="border rounded px-1.5 py-0.5 text-xs">
            {s}
          </span>
        ))}
      </div>
      <div className="px-3 pb-3 mt-auto flex items-center justify-between border-t pt-3">
        ...
        <button
          ...
          className={`border rounded px-3 py-2 text-sm ${
            inWishlist ? "bg-pink-100 border-pink-400 text-pink-600" : ""
          }`}
          aria-label={
            inWishlist ? "Remover da wishlist" : "Adicionar à wishlist"
          }
        >
          {inWishlist ? "♥" : "♡"}
        </button>
